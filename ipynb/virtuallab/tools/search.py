...
def search_best_coef(calc_result, target, nominal_list):
    """粗搜索：在 nominal_list 中找到最优参数"""
    cost_list = []

    for nominal in nominal_list:
        cost = cost_function(nominal, calc_result, target)
        cost_list.append(cost)

    # 找到最优点
    min_idx = np.argmin(cost_list)
    best_param = nominal_list[min_idx]
    best_cost = cost_list[min_idx]

    # 绘制结果
    plt.figure(figsize=(6, 4))
    plt.plot(nominal_list, cost_list, marker='o')
...
def search(calc_result, target, nominal_list):
    # 先粗搜索
    best_param, best_cost = search_best_coef(calc_result, target, nominal_list)
...
if __name__ == "__main__":
    ...
    nominal_list = np.linspace(0, 5, 20)
    search(calc_result, target, nominal_list)